...
snapentry_expire(void *data)
{
	...
	(void) zfsctl_snapshot_unmount(objsetid, MNT_EXPIRE);
...
zfsctl_snapdir_remove(struct inode *dip, char *name, cred_t *cr, int flags)
{
	...
	error = zfsctl_snapshot_unmount(dmu_objset_id(zsb->z_os), MNT_FORCE);
...
int
zfsctl_snapshot_unmount(uint64_t objsetid, int flags)
{
